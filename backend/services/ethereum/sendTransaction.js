...
let providerFunctions = {
    infura: sendOverholdTransaction,
...
async function sendOverholdTransaction(transactionInfo, pName) {
    ...
    if (fromAddressBalanceBN.isGreaterThanOrEqualTo(totalAmount)) {
        ...
        let url = `${providers.ethereum[pName].general}`;


        let errorMessage;
        let txid;

        await createTransaction.createEthereumTransaction(transactionInfo, url, dataUtils.cryptoCurrencyKeys.ethereum).then(result => {
            txid = result;
        }, err => {
            errorMessage = err;
