...
let providerFunctions = {
	overholdLitecore: getOverholdBalancesLitecore,
	blockcypher: litecoinBlockcypherGetBalances
};

async function getOverholdBalancesLitecore(addresses) {
	let requestBody = {'addresses':addresses};
    let promise = providers.requests.postRequest(requestBody , providers.litecoin.overhold.getBalance);
...
async function litecoinBlockcypherGetBalances(addresses) {
	let promises = providers.requests.getRequestParam(addresses, providers.litecoin.blockcypher.getBalance, dataUtils.serviceStings.balance);
