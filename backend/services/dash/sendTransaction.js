...
let providerFunctions = {
	...
	insightDash: sendOverholdTransaction
};
...
async function sendOverholdTransaction(userCoin, transactionInfo, generateAddress, pName) {
    ...
    let urls = {
        utxo: `${providers.dash[pName].getUTXO}`,
        broadcastTransaction: `${providers.dash[pName].sendTx}`
    };
    transactionInfo.fee = utils.convertToSatoshis(transactionInfo.fee) / dataUtils.coinSettings.toSatoshisPerByteDivisor; // satoshis per kilobyte converted to satoshis per byte
