...
let providerFunctions = {
	overhold: getOverholdBalancesDashcore,
	blockcypher: dashBlockcypherGetBalances
};

async function getOverholdBalancesDashcore(addresses) {
	let requestBody = {'addresses':addresses};
	let promise = providers.requests.postRequest(requestBody, providers.dash.overhold.getBalance);
...
async function dashBlockcypherGetBalances(addresses) {
	let promises = providers.requests.getRequestParam(addresses, providers.dash.blockcypher.getBalance, dataUtils.serviceStings.balance);
