...
async function sendOverholdTransaction(userCoin, transactionInfo, generateAddress, pName) {
    ...
    let urls = {
        utxo: `${providers.dogecoin[pName].getUtxo}`,
        broadcastTransaction: `${providers.dogecoin[pName].sendTx}`
    };
    transactionInfo.fee = utils.convertToSatoshis(transactionInfo.fee) / dataUtils.coinSettings.toSatoshisPerByteDivisor;
